...
- **Agent Callbacks**: Lifecycle logging for blog_agent start/end, model calls, and tool invocations
...
|-----------|--------|---------|-------------|
| `service.name` | `AGENT_NAME` env var | `your-blog_agent-name` | Service identifier (set explicitly in `.env`) |
...
| `service.version` | `K_REVISION` env var | `your-blog_agent-name-00042-abc` (deployed) or `local` (dev) | Cloud Run revision or local dev indicator |
...
`LoggingCallbacks` (in `callbacks.py`) logs blog_agent lifecycle events (start/end, model calls, tool invocations) with automatic trace context correlation.
...
- [Vertex AI | Agent Engine | Trace an Agent](https://cloud.google.com/vertex-ai/generative-ai/docs/blog_agent-engine/manage/tracing)
- [Google Cloud Observability | Instrument ADK Applications with OpenTelemetry](https://cloud.google.com/stackdriver/docs/instrumentation/ai-blog_agent-adk)
