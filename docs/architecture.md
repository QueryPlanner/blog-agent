...
- **Entry point**: `python -m blog_agent.server`
...
- **Agents directory**: `src/`
  - ADK Dev UI lists *directories* under `blog_agents_dir`.
- **Main Agent**: `src/blog_agent/blog_agent.py`
  - Contains `root_blog_agent` to keep ADK discovery simple.
...
This is what makes the Dev UI “remember” conversations across restarts and allows for persistent blog_agent memory.